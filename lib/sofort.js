	...
	async.waterfall([
			...
			function (body, callback) {
				parseXmlString(body, function (err, result) {
					if (err) {
						return callback(new SofortException(err));
					}

					if (result !== null && result.errors) {
						return callback(new SofortCodeException(result.errors.error[0].code));
						...
						return callback(new SofortException('Unknown error'));
					}
...
Sofort.prototype.postData = function (PaymentXml, cb) {
	...
	request({
		...
		return cb(new SofortException('postData failed with statusCode ' + response.statusCode));
	});
};
			...
			function (body, callback) {
				parseXmlString(body, function (err, result) {
					...
					if (result !== null && result.errors) {
						...
						return callback(new SofortException('Invalid transaction ID'));
					} else {
						return callback(new SofortException('Unknown error'));
					}
